...
<div class="modern-container">
    <div class="form-card">
        <h2 class="form-title">Modifier l'horaire</h2>
        
        <div class="alert alert-info">
            <i class="fas fa-info-circle me-2"></i> Avant de modifier un horaire, assurez-vous que la salle et le formateur sont disponibles à ce moment-là.
        </div>
        
        <form action="{{ route('emploi-du-temps.update', $emploiDuTemp->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('formation_id') is-invalid @enderror" id="formation_id" name="formation_id" required>
                    ...
                    @foreach ($formations as $formation)
                        <option value="{{ $formation->id }}" {{ old('formation_id', $emploiDuTemp->formation_id) == $formation->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('salle_id') is-invalid @enderror" id="salle_id" name="salle_id" required>
                    ...
                    @foreach ($salles as $salle)
                        <option value="{{ $salle->id }}" {{ old('salle_id', $emploiDuTemp->salle_id) == $salle->id ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('jour') is-invalid @enderror" id="jour" name="jour" required>
                    ...
                    @foreach ($jours as $jour)
                        <option value="{{ $jour }}" {{ old('jour', $emploiDuTemp->jour) == $jour ? 'selected' : '' }}>
                            {{ ucfirst($jour) }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="time" class="form-control @error('heure_debut') is-invalid @enderror" id="heure_debut" name="heure_debut" value="{{ old('heure_debut', \Carbon\Carbon::parse($emploiDuTemp->heure_debut)->format('H:i')) }}" required>
                    @error('heure_debut')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="time" class="form-control @error('heure_fin') is-invalid @enderror" id="heure_fin" name="heure_fin" value="{{ old('heure_fin', \Carbon\Carbon::parse($emploiDuTemp->heure_fin)->format('H:i')) }}" required>
                    @error('heure_fin')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control @error('date_debut') is-invalid @enderror" id="date_debut" name="date_debut" value="{{ old('date_debut', $emploiDuTemp->date_debut->format('Y-m-d')) }}" required>
                    @error('date_debut')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control @error('date_fin') is-invalid @enderror" id="date_fin" name="date_fin" value="{{ old('date_fin', $emploiDuTemp->date_fin ? $emploiDuTemp->date_fin->format('Y-m-d') : '') }}">
            ...
            <div class="mb-4 form-check">
                <input type="checkbox" class="form-check-input" id="est_actif" name="est_actif" value="1" {{ old('est_actif', $emploiDuTemp->est_actif) ? 'checked' : '' }}>
            ...
            <div class="d-flex justify-content-between">
                <div>
                    ...
                    @include('components.back-button', ['url' => route('emploi-du-temps.show', $emploiDuTemp->id), 'text' => 'Retour aux détails'])
                </div>
                
                <button type="submit" class="btn-submit">
                    <i class="fas fa-check-lg me-1"></i>Mettre à jour
                </button>
            </div>
        </form>
    </div>
