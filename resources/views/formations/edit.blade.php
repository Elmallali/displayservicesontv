        ...
        <form action="{{ route('formations.update', $formation->id) }}" method="POST">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('niveau_langue') is-invalid @enderror" id="niveau_langue" name="niveau_langue" required>
                        ...
                        @foreach ($niveauxLangue as $code => $niveau)
                            <option value="{{ $code }}" {{ old('niveau_langue', $formation->niveau_langue ?? '') == $code ? 'selected' : '' }}>{{ $code }} - {{ $niveau }}</option>
                        @endforeach
                    </select>
                    @error('niveau_langue')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('langue') is-invalid @enderror" id="langue" name="langue" required>
                        ...
                        @foreach ($langues as $langue)
                            <option value="{{ $langue }}" {{ old('langue', $formation->langue ?? '') == $langue ? 'selected' : '' }}>{{ ucfirst($langue) }}</option>
                        @endforeach
                    </select>
                    @error('langue')
                        <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <input type="number" step="0.01" min="0" class="form-control @error('prix_mensuel') is-invalid @enderror" id="prix_mensuel" name="prix_mensuel" value="{{ old('prix_mensuel', $formation->prix_mensuel) }}" required>
                    @error('prix_mensuel')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" min="1" class="form-control @error('duree_mois') is-invalid @enderror" id="duree_mois" name="duree_mois" value="{{ old('duree_mois', $formation->duree_mois) }}" required>
                    @error('duree_mois')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" min="1" class="form-control @error('places_maximum') is-invalid @enderror" id="places_maximum" name="places_maximum" value="{{ old('places_maximum', $formation->places_maximum) }}" required>
