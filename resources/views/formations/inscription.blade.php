@extends('layouts.app')

@section('content')
<style>
    :root {
        --primary: #3563E9;
        --secondary: #121A2D;
        --accent: #4FBAEE;
        --background: #1C3139;
        --card-bg: rgba(51, 148, 205, 0.18);
        --text: #f0f0f0;
        --gradient: linear-gradient(135deg, #3394cd, #82CEF9);
        --hover-gradient: linear-gradient(135deg, #1c3139, #2a4b57);
    }

    body {
        background: var(--background);
        color: var(--text);
        font-family: 'Segoe UI', sans-serif;
    }

    .modern-container {
        padding: 3rem 1rem;
        max-width: 800px;
        margin: auto;
    }

    .form-card {
        background: var(--card-bg);
        backdrop-filter: blur(10px);
        border: 1px solid rgba(255, 255, 255, 0.1);
        border-radius: 15px;
        overflow: hidden;
        box-shadow: 0 8px 20px rgba(0,0,0,0.25);
        padding: 30px;
    }

    .form-title {
        font-size: 1.8rem;
        font-weight: 600;
        margin-bottom: 25px;
        color: var(--accent);
    }

    .form-subtitle {
        font-size: 1.1rem;
        color: #ddd;
        margin-bottom: 25px;
        padding-bottom: 15px;
        border-bottom: 1px solid rgba(255, 255, 255, 0.1);
    }

    .form-control {
        background-color: rgba(255, 255, 255, 0.1);
        border: 1px solid rgba(255, 255, 255, 0.2);
        color: var(--text);
        border-radius: 8px;
        padding: 12px 15px;
    }

    .form-control:focus {
        background-color: rgba(255, 255, 255, 0.15);
        border-color: var(--accent);
        color: var(--text);
        box-shadow: 0 0 0 0.25rem rgba(79, 186, 238, 0.25);
    }

    .form-label {
        color: #ddd;
        font-weight: 500;
        margin-bottom: 8px;
    }

    .form-select {
        background-color: rgba(255, 255, 255, 0.1);
        border: 1px solid rgba(255, 255, 255, 0.2);
        color: var(--text);
        border-radius: 8px;
        padding: 12px 15px;
    }

    .form-select:focus {
        background-color: rgba(255, 255, 255, 0.15);
        border-color: var(--accent);
        color: var(--text);
        box-shadow: 0 0 0 0.25rem rgba(79, 186, 238, 0.25);
    }

    .btn-modern {
        border: 1px solid var(--accent);
        background: transparent;
        color: var(--accent);
        padding: 10px 20px;
        border-radius: 8px;
        font-size: 1rem;
        transition: background 0.3s, color 0.3s;
        margin-right: 10px;
        text-decoration: none;
    }

    .btn-modern:hover {
        background: var(--accent);
        color: #fff;
    }

    .btn-submit {
        background: var(--gradient);
        color: white;
        border: none;
        padding: 12px 25px;
        border-radius: 8px;
        font-weight: 600;
        transition: background 0.4s ease;
    }

    .btn-submit:hover {
        background: var(--hover-gradient);
    }

    .invalid-feedback {
        color: #ff6b6b;
        font-size: 0.85rem;
    }
</style>

<div class="modern-container">
    <div class="form-card">
        <h2 class="form-title">Inscrire un élève à la formation</h2>
        <p class="form-subtitle">
            <strong>Formation:</strong> {{ $formation->nom }} ({{ $formation->niveau }})<br>
            <strong>Formateur:</strong> {{ $formation->formateur->nom }}<br>
            <strong>Prix mensuel:</strong> {{ $formation->prix_mensuel }} DH
        </p>
        
        @if (session('success'))
            <div class="alert alert-success">{{ session('success') }}</div>
        @endif
        
        @if (session('error'))
            <div class="alert alert-danger">{{ session('error') }}</div>
        @endif
        
        @if ($eleves->isEmpty())
            <div class="alert alert-info">
                Tous les élèves sont déjà inscrits à cette formation ou aucun élève n'est enregistré dans le système.
                <a href="{{ route('eleves.create') }}" class="alert-link">Ajouter un nouvel élève</a>
            </div>
        @else
            <form action="{{ route('formations.inscription', $formation->id) }}" method="POST">
                @csrf
                
                <div class="mb-3">
                    <label for="eleve_id" class="form-label">Élève <span class="text-danger">*</span></label>
                    <select class="form-select @error('eleve_id') is-invalid @enderror" id="eleve_id" name="eleve_id" required>
                        <option value="" disabled selected>Sélectionnez un élève</option>
                        @foreach ($eleves as $eleve)
                            <option value="{{ $eleve->id }}" {{ old('eleve_id') == $eleve->id ? 'selected' : '' }}>
                                {{ $eleve->nom }} {{ $eleve->prenom }} - {{ $eleve->langue_suivie }}
                            </option>
                        @endforeach
                    </select>
                    @error('eleve_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                
                <div class="mb-4">
                    <label for="date_inscription" class="form-label">Date d'inscription <span class="text-danger">*</span></label>
                    <input type="date" class="form-control @error('date_inscription') is-invalid @enderror" id="date_inscription" name="date_inscription" value="{{ old('date_inscription', date('Y-m-d')) }}" required>
                    @error('date_inscription')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                
                <div class="d-flex justify-content-between">
                    <a href="{{ route('formations.eleves', $formation->id) }}" class="btn-modern">
                        <i class="bi bi-arrow-left me-1"></i>Annuler
                    </a>
                    
                    <button type="submit" class="btn-submit">
                        <i class="bi bi-check-lg me-1"></i>Inscrire l'élève
                    </button>
                </div>
            </form>
        @endif
    </div>
</div>
@endsection
